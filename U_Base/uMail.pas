...
begin
  ...
  if not RunCommand('xdg-open', [MailToLink], Result) then // Linux
    if not RunCommand('open', [MailToLink], Result) then   // macOS
      RunCommand('cmd', ['/c', 'start', MailToLink], Result); // Windows
end;{$ELSE}
implementation
uses Mapi;
