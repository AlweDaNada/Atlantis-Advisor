...
begin
  ...
  with AUnit do begin
   ...
   // Fill messages
    ...
    lMonthOut.Caption := '';
    {$ifdef LAZ}
    if imgMonthOrder.Picture.Bitmap<>nil then
      begin
        imgMonthOrder.Picture.Bitmap.FreeImage;
        imgMonthOrder.Picture.Bitmap.SetSize(0,0);
      end;
    {$endif}
    PrepareBmp(imgMonthOrder.Picture.Bitmap, 16, 16, clNether);
    {$ifdef LAZ}
    imgMonthOrder.Picture.Bitmap.TransparentColor:=clNether;
    imgMonthOrder.Picture.Bitmap.TransparentMode:=tmFixed;
    imgMonthOrder.Picture.Bitmap.Transparent:=true;
    {$endif}
    if Faction.Player then begin
...
begin
  ...
  if Sender is TEdit then
      TEdit(Sender).BorderStyle := bsSingle;
  {$ELSE}
...
begin
  ...
  if Sender is TEdit then
    if Sender is TEdit then
        TEdit(Sender).BorderStyle := bsNone;
  {$ELSE}
