...
begin
  ...
  Bmp.Canvas.Brush.Color:=clNether;
  ...
  if Shadow then begin
    ...
    DrawTranspText(Bmp.Canvas, 0, 0, s, MaxWidth, False);
  end
...
{$ifdef LAZ}
function GetMaskedBmp(List: TImageList; Index: integer; Color: TColor): TBitmap;
var _msk, _img: TBitmap;
    _clTransp : TColor;
begin
  if Color = clBlack then Color := clNether;
  if Color <> clFuchsia then _clTransp := clFuchsia
  else _clTransp := clOlive;
  // prepare extra image
  _img := TBitmap.Create;
  _img.SetSize(List.Width, List.Height);
  _img.Canvas.Brush.Color := _clTransp;
  _img.Canvas.FillRect(Rect(0, 0, _img.Width, _img.Height));
  ResForm.Extras.Draw(_img.Canvas,0,0,Index*2);
  // prepare mask
  _msk := TBitmap.Create;
  _msk.SetSize(List.Width, List.Height);
  _msk.Canvas.Brush.Color := Color;
  _msk.Canvas.FillRect(Rect(0, 0, _msk.Width, _msk.Height));
  ResForm.Extras.Draw(_msk.Canvas,0,0,Index*2+1);
  // draw mask on image with transparent
  _msk.TransparentColor:=_msk.Canvas.Pixels[0,_msk.Height-1];
  _msk.TransparentMode:=tmFixed;
  _msk.Transparent := TRUE;
  _img.Canvas.Draw(0, 0, _msk);
  // prepare final image
  _img.TransparentColor:=_clTransp;
  _img.TransparentMode:=tmFixed;
  _img.Transparent := TRUE;
  _msk.Free;
  List.BkColor := clNone;
  Result := _img;
end;
...
begin
  if Flip then begin
    ...
    {$ifdef LAZ}
    Bmp1.TransparentColor:=Bmp1.Canvas.Pixels[0,Bmp1.Canvas.Height-1];
    ...
    {$ifdef LAZ}
    Bmp.TransparentColor:=Bmp.Canvas.Pixels[0,Bmp.Canvas.Height-1];
...
var
    _img : TBitmap;
begin
  _img := TColorExtra(ColorExtras[FactionCIndex(AFaction)]).Images[Index];
  ACanvas.Draw(x, y, _img);
end;
{$else}
procedure DrawCExtra(Index: integer; AFaction: TFaction; ACanvas: TCanvas; x, y: integer);
begin
...
begin
  if Filename = '' then begin
    ...
    Bmp := CreateUnitBmp(clNether); //old
    ...
    while not Trace.Ends do begin
      ...
      if s <> '' then begin
        if Pos('+', s) > 0 then begin
          ...
          {$ifdef LAZ}
          MaskBmp.TransparentColor:=MaskBmp.Canvas.Pixels[0,MaskBmp.Canvas.Height-1];
          MaskBmp.TransparentMode:=tmFixed;
          {$endif}
