...
type
  ...
  protected
    ...
    procedure DrawHex(HX, HY: integer);
    procedure DrawExtra(HX, HY: integer);
    {$ELSE}
  ...
  public
    ...
    procedure Hex(ACanvas: TCanvas; CX,CY: integer);
    ...
    procedure PartialHex(ACanvas: TCanvas; CX,CY: integer; Parts: THexParts);
    procedure Redraw;
...
{$IFDEF LAZ}
function TCylinderMap.DoKeyDown(var Message: TLMKey): Boolean;
begin
     inherited DoKeyPress(Message);
     Result := FALSE;
end;
...
procedure TCylinderMap.Hex(ACanvas: TCanvas; CX,CY: Integer);
begin
...
begin
  MapRect := Rect(
             X,
             Y,
             X + FMapRect.Right - FMapRect.Left,
             Y + FMapRect.Bottom - FMapRect.Top);
  if Assigned(FOnMoveMap) then
    FOnMoveMap(Self,X,Y);
end;
...
begin
  if not (msNoPaint in MapState) then begin
    BeforePaint;
    if msRedraw in MapState then
      Redraw
    else
      Canvas.CopyRect(
         Rect(DrawOffset.X, DrawOffset.Y, Width - DrawOffset.X, Height - DrawOffset.Y),
         OffScreen.Canvas,
         Rect(hs + DrawOffset.X, hs43*2 + DrawOffset.Y, hs+Width - DrawOffset.X, hs43*2+Height - DrawOffset.Y)
      );
    AfterPaint;
  end;
end;

procedure TCylinderMap.PartialHex(ACanvas: TCanvas; CX,CY: Integer; Parts: THexParts);
begin
  with ACanvas do begin
    if Parts[dirS] then
      begin
        MoveTo(CX - hs25, CY + hs43);
        LineTo(CX + hs25, CY + hs43);
      end;
    if Parts[dirSW] then
      begin
        MoveTo(CX - hs50, CY);
...
begin
 ...
 { Draw hexes on off-screen bitmap }
  ...
  {$IFDEF LAZ}
  IterateHexes(FVisHexes, TIterationHandler(DrawHex));
 { Draw extras }
  IterateHexes(FVisCells, TIterationHandler(DrawExtra));
  {$ELSE}
...
begin
  if CellVisible(HX,HY) then begin
   ...
   { Restore neibors' extras removed by drawn hex }
    Extras := Rect(
        HX - FExtraSpan.X,
        Max(HY-FExtraSpan.Y, 0),
        HX + FExtraSpan.X,
        Min(HY+FExtraSpan.Y, RowCount-1));
    {$IFDEF LAZ}
    IterateHexes(Extras, TIterationHandler(DrawExtra));
    {$ELSE}
...
begin
  ...
  RecalcMapRect;
  {$IFDEF LAZ}
  IterateHexes(FVisHexes, TIterationHandler(IHDrawNewHex));
  // Redraw extras
  IterateHexes(FVisCells, TIterationHandler(IHDrawNewCell));
  {$ELSE}
