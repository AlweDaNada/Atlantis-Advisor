...
uses
  ...
  Grids, Menus, IniFiles, Math, StdCtrls
  {$IFDEF LAZ}
...
type
  ...
  {$IFDEF LAZ}
  TIntArray = array[0..$7FFFFFF] of Integer;
  {$ELSE}
...
begin
  case Key of
    ...
    VK_INSERT:
      ...
      {$IFDEF LAZ}
      else if (Shift = [ssShift]) and not Grid.Editing then Key := 0;
      {$ELSE}
    ...
    VK_DELETE:
      if Ctrl then
        ...
        {$IFDEF LAZ}
        if not Grid.Editing then Key := 0;
        {$ELSE}
...
begin
  ...
  {$ELSE}
  if (Key in [#32..#255]) and not Grid.CanCanEditAcceptKey(Key) then
  {$ENDIF}
  begin
    Key := #0;
    MessageBeep(0);
...
begin
  ...
  if not UpdatingColWidths then begin
    for i := 0 to ImgColCount-1 do
      ImgCols[i].FWidth := ColWidths[i];
  end;
  ResizeRows;
end;
...
begin
  GlobalPos := Mouse.CursorPos;
  LocalPos := ScreenToClient(GlobalPos);
  Result := MouseCoord(LocalPos.X, LocalPos.Y);
end;
...
begin
  ...
  GlobalPos := Mouse.CursorPos;
  LocalPos := ScreenToClient(GlobalPos);
  if LocalPos.X < Rect.Left + (Rect.Right - Rect.Left) div 2 then Half := FALSE
  else Half := TRUE;
end;
...
begin
  if Canvas.TextWidth(s) <= Wid then
    begin
    Result := s;
    Count := 1;
    end
